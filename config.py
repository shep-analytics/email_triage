...
# client_secret_desktop.json for local CLI auth, but the web UI requires a
# Google OAuth client of type "Web application" (JSON with a top‑level
# "web" key). Point this to the web client so the UI can present the
# "Connect Gmail" flow for re‑consent and token refresh when scopes change.
#
# If you prefer to set this via env, use GMAIL_OAUTH_CLIENT_SECRET.
GMAIL_CLIENT_SECRET_PATH = "json_keys/client_secret.json"
